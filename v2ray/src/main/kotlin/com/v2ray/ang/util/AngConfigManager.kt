...
object AngConfigManager {
  ...
  private fun importConfig(
    ...
    try {
      ...
      if (TextUtils.isEmpty(subid) && (
          str.startsWith(HTTP_PROTOCOL) || str.startsWith(
            HTTPS_PROTOCOL,
          )
          )
      ) {
  ...
  private fun tryResolveResolveSip002(str: String, config: ServerConfig): Boolean {
    try {
      ...
      Log.d(AppConfig.ANG_LOGS_TAG, e.toString())
      return false
    }
  }
      ...
      return config.configType.protocolScheme + when (config.configType) {
        ...
        -> {
          ...
          if (config.configType == EConfigType.VLESS) {
            ...
            dicQuery["encryption"] =
              if (outbound.getSecurityEncryption().isNullOrEmpty()) {
                "none"
              } else {
                outbound.getSecurityEncryption().orEmpty()
              }
          } else if (config.configType == EConfigType.TROJAN) {
