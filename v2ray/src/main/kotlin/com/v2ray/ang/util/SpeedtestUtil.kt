...
object SpeedtestUtil {
  ...
  fun realPing(config: String): Long {
    return try {
      ...
      Log.d(AppConfig.ANG_LOGS_TAG, "realPing: $e")
      -1L
    }
  }
  ...
  fun socketConnectTime(url: String, port: Int): Long {
    try {
      ...
      Log.d(AppConfig.ANG_LOGS_TAG, "socketConnectTime IOException: $e")
    } catch (e: Exception) {
      e.printStackTrace()
    }
    return -1
  ...
  fun testConnection(context: Context, port: Int): String {
    ...
    try {
      ...
      // network exception
      Log.d(AppConfig.ANG_LOGS_TAG, "testConnection IOException: " + Log.getStackTraceString(e))
      result = context.getString(R.string.connection_test_error, e.message)
    } catch (e: Exception) {
      // library exception, eg sumsung
      Log.d(AppConfig.ANG_LOGS_TAG, "testConnection Exception: " + Log.getStackTraceString(e))
      result = context.getString(R.string.connection_test_error, e.message)
    } finally {
      conn?.disconnect()
    }
